...
var Spider = function(obj) {
	...
	// 匹配字符串
	this.matchStr = obj.matchStr || "";
	//爬取文件白名单
	this.whiteTypeList = obj.whiteTypeList || [];
	this.startTime = null;
	//第一个URL地址入未抓取队列
	if (this.firstUrl) {
	...
	//辅助对象
	this.oUrl = new Url();
	this.oFile = this.saveDir && new File({
 ...
 * 开始抓取
 */
Spider.prototype.crawl = function(callback) {
	if(callback){
	...
	this.debug && console.log("quene length: ", aNewUrlQueue.length)
	if (aNewUrlQueue.length > 0) {
		var urlone = aNewUrlQueue.pop();
		this.debug && console.log('parse url ', urlone)
		this.sendReq(urlone);
		oCnt.total++;
		...
		oCnt.time = Date.now()-this.startTime;
		this.callback(oCnt);
	}
	return true;
}
...
Spider.prototype.sendReq = function(urlone) {
	...
	req.on('response', function(res) {
		...
		var data = '';
		if(self.whiteTypeList.length && self.whiteTypeList.indexOf(aType[1]) == -1){
			req.abort();
		}
		if (aType[2] !== "binary") {
...
Spider.prototype.parseUrl = function(html) {
	...
	for (var i = 0; i < aRegex.length; i++) {
		do {
			...
			if (aRet) {
				this.debug && this.oFile && this.oFile.save("_log/aParseUrl.log", aRet.join("\n") + "\n\n", "utf8", function() {}, true);
...
Spider.prototype.handlerSuccess = function(str, aType, urlone) {
	...
	if (aType[0] === "text" && (!this.whiteTypeList.length || this.whiteTypeList.length  && this.whiteTypeList.indexOf(aType[1]) > -1)) { //提取URL地址
		...
		for (var i = 0; i < aUrls.length; i++) {
			...
			if (!this.oUrl.isValidPart(aUrls[i])) {
				this.debug && this.oFile && this.oFile.save("_log/aInvalidRawUrl.log", urlone + "----" + aUrls[i] + "\n", "utf8", function() {}, true);
				continue;
			}
	...
	if (sPath) {
		if (this.debug && this.oFile) {
			this.oFile.save("_log/urlFileSave.log", urlone + "--------" + sPath + "\n", "utf8", function() {}, true);
		}
		if (sPath.length > sPathMaxSize) { //可能有问题的路径 打监控日志
			this.oFile && this.oFile.save("_log/sPathMaxSizeOverLoad.log", urlone + "--------" + sPath + "\n", "utf8", function() {}, true);
			return;
		}
		if (aType[2] != "binary") { //只支持UTF8编码
			aType[2] = "utf8";
		}
		if (self.matchStr && str.indexOf(self.matchStr) > -1) {
			if(this.oFile){
